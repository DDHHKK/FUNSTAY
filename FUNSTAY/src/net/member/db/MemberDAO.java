...
//ȸ������ getMemberList()�Լ�
	...
	public int[] gettotalMileage(String member_email){
		...
		try{
			con=getConnection();
			String sql = "select sum(storage_m) as total_m, sum(used_m) as use_m from payment where member_email=? && payment_status='�����Ϸ�'";
			pstmt = con.prepareStatement(sql);
	...
	// QnA����Ʈ
		public List<QnaBean> getQnAList(String member_email){
			...
			try{
				con = getConnection();
				String sql = "select * from qna_board where member_email=? order by QnA_num desc";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, member_email);
